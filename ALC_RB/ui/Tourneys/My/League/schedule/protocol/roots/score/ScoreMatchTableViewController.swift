...
//        initPresenter()
        ...
        //match.events.append(Event(id: "apsifjsd0890", eventType: "yellowCard", player: "5be94d0206af116344942a58", time: "2 тайм"))
...
//
//        configureFooter(league: leagueDetailModel.league, match: match)
//    }
//
...
//
//        presenter.getClubImage(id: ClubTeamHelper.getClubIdByTeamId(match.teamOne.getId() ?? match.teamOne.getValue()!.id!, league: league)) { (image) in
...
//
//        configureCell(cell: cell, league: leagueDetailModel.league, match: match, event: tableStruct.events[indexPath.section][indexPath.row])
//
//        return cell
//    }
//
//    func configureCell(cell: ScoreMatchTableViewCell, league: League, match: Match, event: Event) {
//
//        let playerId = event.player
////        print ("playerId =  \(playerId)")
////        print ("team One = \(match.teamOne.getId() ?? match.teamOne.getValue()!.id) --- team Two = \(match.teamTwo)")
////        print(getTeamIdByPlayerId(league: league, match: match, player: playerId))
//
//        if getTeamIdByPlayerId(league: league, match: match, player: playerId) == match.teamOne.getId() ?? match.teamOne.getValue()!.id {
...
//        let playerId = id
//        let teamOne = match.teamOne.getId() ?? match.teamOne.getValue()!.id
